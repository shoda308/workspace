...
saved_model_pth = '/workspace/results/2024_06/'+ args.risk_factor + '/model/mixup/'

os.makedirs(results_file, exist_ok=True)
os.makedirs(results_file + '/log', exist_ok=True)
os.makedirs(results_file + '/log/mixup', exist_ok=True)
os.makedirs(results_file + '/predscore', exist_ok=True)
os.makedirs(results_file + '/predscore/mixup', exist_ok=True)
...
logname = (results_file +'/log/mixup/seed=' + str(args.seed) + '_log_' + dt_now.strftime('%Y_%m_%d_%H_%M_%S') + args.risk_factor + '.csv')
...
def train(epoch, X, Y):
    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        inputs, targets, targets_a, targets_b, lam = mixup_data(inputs, targets, args.alpha)#targets_a:正解ラベル, targets_b:仮想的な正解ラベル
        optimizer.zero_grad()# 勾配リセット
        inputs,targets, targets_a, targets_b = Variable(inputs),  Variable(targets), Variable(targets_a), Variable(targets_b)
    ...
    if epoch == start_epoch + args.epoch - 1:
        ...
        model_num += 1
        save_model(net, optimizer, epoch, saved_model_pth +str(model_num)+'_mixup.pth')
...
def mixup_data(x, y, alpha):
    ...
    if alpha > 0:
        np.random.seed(args.seed)  # NumPyの乱数生成器のシードを設定
        lam = np.random.beta(args.alpha, args.alpha)  # ランダムな混合比率を生成 seed揃える
    else:
        lam = 1
    # lam = 0.2958255840891495
    print("\nlam:",lam)
    # exit(1)

    batch_size = x.size()[0]
    index = torch.randperm(batch_size)
    #x[index, :]：混ぜるための別のデータ　lam * x：元のデータポイントxをlamの重みでスケーリング
    mixed_x = lam * x + (1 - lam) * x[index, :]  # 入力データを混合
    y_a, y_b = y, y[index]  # ターゲットラベルも混合
    mixed_y = lam * y_a + (1 - lam) * y_b
    # x と mixed_x を結合
    inputs = torch.cat((x, mixed_x), dim=0)
    outputs = torch.cat((y, mixed_y), dim=0)
    return inputs,outputs,y_a, y_b, lam
...
with open(results_file +"/predscore/mixup/"+ args.risk_factor +'_seed='  + str(args.seed) + 'mixup_predscore_valid.csv', "w", newline="") as csvfile:
...
with open(results_file +"/predscore/mixup/"+ args.risk_factor +'_seed='  + str(args.seed) + 'mixup_predscore_test.csv', "w", newline="") as csvfile:
                writer = csv.writer(csvfile)
                writer.writerows(result)
